...
router.get("/index", async function (req, res, next) {
  ...
  subLike.forEach((value) => {
    likeResult.push({ private_id: value, count: 0 });
  });
  likeResult.sort((a, b) => {
  ...
  subView.forEach((value) => {
    viewResult.push({ private_id: value, count: 0 });
  });
  viewResult.sort((a, b) => {
    return a.private_id - b.private_id;
  });
  ...
  subComment.forEach((value) => {
    commentResult.push({ private_id: value, count: 0 });
  });
...
router.get("/test/:id", async function (req, res, next) {
  ...
  let PersonRecipeArr = [];
  PersonRecipe.map((value) => {
    PersonRecipeArr.push(value.id);
  });
