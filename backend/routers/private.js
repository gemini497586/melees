...
router.get("/index", async function (req, res, next) {
    ...
    let view =
        "SELECT private_id, count(*) as count FROM private_view GROUP BY private_id";
...
router.get("/index/:id?", async function (req, res, next) {
    ...
    let sql2 =
        "SELECT * FROM private_follow WHERE user_id = ? AND private_id = ?";
...
router.post(
    ...
    async function (req, res, next) {
        ...
        let sql =
            "INSERT INTO private_recipe (picture, name, intro, qty, member_id, create_date, star_rate, valid) VALUES (?)";
        let data = await connection.queryAsync(sql, [
            [filename, name, intro, qty, memberId, time, star_rate, valid],
        ]);
        ...
        ingred.forEach(async (value) => {
            let ingredList =
                "INSERT INTO private_ingred (private_id, ingred, ingred_unit) VALUES (?)";
            let ingredResult = await connection.queryAsync(ingredList, [
                [lastId[0].id, value.ingred, value.ingred_unit],
            ]);
        });
        ...
        steps.forEach(async (value) => {
            let stepList =
                "INSERT INTO private_step (private_id, steps) VALUES (?)";
            let stepResult = await connection.queryAsync(stepList, [
                [lastId[0].id, value.step],
            ]);
        });
...
router.post("/comment/upload/:id", async function (req, res, next) {
    ...
    let data = await connection.queryAsync(sql, [
        [req.params.id, memberId, comment, star_rate, comment_time],
    ]);
    ...
    let result2 = await connection.queryAsync(sql3, [
        averageStar,
        req.params.id,
    ]);
