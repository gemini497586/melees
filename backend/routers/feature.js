...
router.get("/step/:listId?", async function (req, res, next) {
  ...
  let sql = " SELECT a.id AS listId, b.id AS stepId, b.feature_id, b.step AS steps FROM feature_list AS a INNER JOIN feature_step AS b ON a.id=b.feature_id WHERE a.id=?"
...
router.get("/prep/:listId?", async function (req, res, next) {
  ...
  let sql = " SELECT a.id AS listId, b.id AS stepId, b.feature_id, b.prep AS ingred, b.unit AS ingred_unit FROM feature_list AS a INNER JOIN feature_prep AS b ON a.id=b.feature_id WHERE a.id=?"
...
router.post("/weeklist/", async function (req, res, next) {
  ...
  let sql1 = "SELECT a.id AS listId, a.type_id, a.name AS listName, b.id AS weeklistId, b.week_id AS weekid, b.feature_id AS weekfeatureid, b.order_date, c.id AS weekId, c.first_date AS firstdate FROM feature_list AS a INNER JOIN feature_weeklist AS b ON a.id=b.feature_id INNER JOIN feature_week AS c ON b.week_id=c.id"
